        ...
        for (let index = 0; index < selectBoxes.length; index++) {
            ...
            selected.addEventListener('click', () => {
                if (!optionsContainer.classList.contains('_active')) {
                    selectBoxes.forEach((el) => {
                        ...
                        el.querySelector('.selected').classList.remove('_active');
                    });
                }
            ...
            optionsList.forEach(o => {
                o.addEventListener('click', (e) => {
                    if (o.closest('.select-box_inner')) {
                        selected.innerHTML = o.querySelector('label').innerHTML;
                    } else {
                        selected.innerHTML = o.querySelector('input').value;
                    }
...
/* harmony import */ var _modules_page_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(2);
/* harmony import */ var _modules_form_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(3);
/* harmony import */ var apexcharts__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(4);
...
var chertOptions = {
    ...
    xaxis: {
        categories: ["1:29 AM", "2:29 AM", "3:29 AM", "4:29 AM", "5:29 AM", "6:29 AM", "7:29 AM", "8:29 AM", "9:29 AM"],
        tickAmount: 2,
        labels: {
            style: {
                colors: '#A8A8A8',
...
if (document.querySelector("#chart1")) {
    var chart1 = new apexcharts__WEBPACK_IMPORTED_MODULE_3__(document.querySelector("#chart1"), chertOptions);
    chart1.render();
}

if (document.querySelector("#chart2")) {
    var chart2 = new apexcharts__WEBPACK_IMPORTED_MODULE_3__(document.querySelector("#chart2"), chertOptions);
    chart2.render();
}
...
function uploadFile(file, input) {
    ...
    const filePreview = fileParent.querySelector('.file__preview');

    if (!['image/jpeg', 'image/png', 'image/pdf'].includes(input.type)) {
        alert('Only jpg, png, pdf file formats');
        return;
    }
    ...
    filePreview.closest('.file').classList.add('active');
    filePreview.querySelector('span').innerHTML = input.name;
}
...
function toggleUserMenu() {
    const userMenu = document.querySelector('.user__body');
...
document.addEventListener('click', (e) => {
    ...
    const mainBtn = e.target.closest('.main__btn');
    if (mainBtn) {
        showAlphabet();
    }

    const closeAlphabetBtn = e.target.closest('.alphabet__close');
    if (closeAlphabetBtn) {
        closeAlphabet();
    }

    const passToggleBtn = e.target.closest('.pass__switcher');
    if (passToggleBtn) {
        passToggle(passToggleBtn);
    }
    ...
    const switcherBtn = e.target.closest('.switcher');
    if (switcherBtn) {
        switcher(switcherBtn);
    }
    ...
    const helpCloseBtn = e.target.closest('.help__close');
    if (helpCloseBtn) {
        hideHelp();
    }

    const file = e.target.closest('.file__body');
    if (file) {
        selectFile(file);
    }

    const fileDeleteBtn = e.target.closest('.file__delete');
    if (fileDeleteBtn) {
        deleteFile(fileDeleteBtn);
    }

    const searchBtn = e.target.closest('.search-header__btn');
    if (searchBtn) {
        showSearchForm(searchBtn);
    }

    const searchClose = e.target.closest('.search-header__close');
    if (searchClose) {
        closeSearchForm();
    }
